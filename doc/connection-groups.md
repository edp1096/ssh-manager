...
2. **Connection groups** toggles a sidebar on the left of Hosts. On narrow windows it overlays part of the list. Use its close button or Escape while focused inside to hide it.
...
Grid fills hosts left to right, then top to bottom, using the group's saved order. Row count is automatic. For example, 3 columns and 6 hosts produce `1,2,3 / 4,5,6`; 9 hosts produce `1,2,3 / 4,5,6 / 7,8,9`. An incomplete last row shares its full width among its remaining hosts; no empty SSH panes are added.
...
Rows have equal height and each row's cells have equal width, subject to terminal minimum sizes, window space and character-cell rounding. The launcher creates row anchors first, so the **connection startup order** can differ from the final visual order. Failure messages identify the host whose launch failed; earlier opened connections remain open.
